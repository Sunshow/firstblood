...
public class DcRaceAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private Logger logger = LoggerFactory.getLogger(DcRaceAction.class);
	...
	private List<DcRace> compareRaces;
	
	private String isSpOrRace; // ajax更新对阵或SP标记

	private String tag;

	private DcRace dcRace;
	...
	public String fetchDcRaces() {
		logger.info("进入获取比赛数据");
		// 比分SP值抓取
		if (pageResult != null && !pageResult.trim().isEmpty()) {
			// TODO 抓取比赛sp和比分
			List<DcRace> dbDcRaces = dcRaceService
					.getDcRaceListByPhase(phaseNo);
			if (dbDcRaces != null && !dbDcRaces.isEmpty()) {
				List<DcRace> fetchDcRaces = dcRaceService
						.fetchDcRaceSpListByPhase(phaseNo, FetcherType
								.getItem(fetcherType));
				if (fetchDcRaces == null || fetchDcRaces.isEmpty()) {
					logger.error("获取比赛数据，赛事结果SP值数据为空");
					super.setErrorMessage("未抓取到赛事结果SP值数据，请尝试其它抓取");
				} else {
					dcRaces = packDcRaces(dbDcRaces, fetchDcRaces);
					if (fetcherTypeCompare == fetcherType) {
						super.setErrorMessage("未比较，抓取地址和比较地址不能为同一地址");
						logger.error("未比较，抓取地址和比较地址不能为同一地址");
					} else {
						List<DcRace> fetchDcRacesCompare = dcRaceService
							.fetchDcRaceSpListByPhase(phaseNo, FetcherType
								.getItem(fetcherTypeCompare));
						if (fetchDcRacesCompare == null || fetchDcRacesCompare.isEmpty()) {
							...
							// 抓取数据遍历
							Map<Integer, DcRace> dcRacesMap = new HashMap<Integer, DcRace>();
							for (DcRace dcRace : fetchDcRacesCompare) {
								dcRacesMap.put(dcRace.getMatchNum(), dcRace);
							}
							compareRaces = new ArrayList<DcRace>();
							for (DcRace dcRace : dcRaces) {
								DcRace tempDcRace = dcRacesMap.get(dcRace.getMatchNum());
								boolean flag = false;
								compareRaces.add(new DcRace()); 
								if (tempDcRace == null) {
									flag = true;
									continue;
								}
								//比较半场比分
								if (!compare(dcRace.getHalfScore(), tempDcRace.getHalfScore())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setHalfScore(tempDcRace.getHalfScore());
								}
								//比较全场比分
								if (!compare(dcRace.getWholeScore(), tempDcRace.getWholeScore())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setWholeScore(tempDcRace.getWholeScore());
								}
								//胜负平sp值
								if ("".equals(dcRace.getSpSfp()) && !"".equals(tempDcRace.getSpSfp())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setSpSfp(tempDcRace.getSpSfp());
								} else if (!"".equals(dcRace.getSpSfp()) && "".equals(tempDcRace.getSpSfp())) {
									flag = true;
								} else if (!"".equals(dcRace.getSpSfp()) && !"".equals(tempDcRace.getSpSfp())) {
									if(Double.compare(Double.parseDouble(dcRace.getSpSfp()), Double.parseDouble(tempDcRace.getSpSfp())) != 0) {
										flag = true;
										compareRaces.get(compareRaces.size() - 1).setSpSfp(tempDcRace.getSpSfp());
									}
								}
								//上下单双sp值
								if ("".equals(dcRace.getSpSxds()) && !"".equals(tempDcRace.getSpSxds())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setSpSxds(tempDcRace.getSpSxds());
								} else if (!"".equals(dcRace.getSpSxds()) && "".equals(tempDcRace.getSpSxds())) {
									flag = true;
								} else if (!"".equals(dcRace.getSpSxds()) && !"".equals(tempDcRace.getSpSxds())) {
									if(Double.compare(Double.parseDouble(dcRace.getSpSxds()), Double.parseDouble(tempDcRace.getSpSxds())) != 0) {
										flag = true;
										compareRaces.get(compareRaces.size() - 1).setSpSxds(tempDcRace.getSpSxds());
									}
								}
								//进球数sp值
								if ("".equals(dcRace.getSpJqs()) && !"".equals(tempDcRace.getSpJqs())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setSpJqs(tempDcRace.getSpJqs());
								} else if (!"".equals(dcRace.getSpJqs()) && "".equals(tempDcRace.getSpJqs())) {
									flag = true;
								} else if (!"".equals(dcRace.getSpJqs()) && !"".equals(tempDcRace.getSpJqs())) {
									if(Double.compare(Double.parseDouble(dcRace.getSpJqs()), Double.parseDouble(tempDcRace.getSpJqs())) != 0) {
										flag = true;
										compareRaces.get(compareRaces.size() - 1).setSpJqs(tempDcRace.getSpJqs());
									}
								}
								//单场比分sp值
								if ("".equals(dcRace.getSpBf()) && !"".equals(tempDcRace.getSpBf())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setSpBf(tempDcRace.getSpBf());
								} else if (!"".equals(dcRace.getSpBf()) && "".equals(tempDcRace.getSpBf())) {
									flag = true;
								} else if (!"".equals(dcRace.getSpBf()) && !"".equals(tempDcRace.getSpBf())) {
									if(Double.compare(Double.parseDouble(dcRace.getSpBf()), Double.parseDouble(tempDcRace.getSpBf())) != 0) {
										flag = true;
										compareRaces.get(compareRaces.size() - 1).setSpBf(tempDcRace.getSpBf());
									}
								}
								//半场胜负平sp值
								if ("".equals(dcRace.getSpBcsfp()) && !"".equals(tempDcRace.getSpBcsfp())) {
									flag = true;
									compareRaces.get(compareRaces.size() - 1).setSpBcsfp(tempDcRace.getSpBcsfp());
								} else if (!"".equals(dcRace.getSpBcsfp()) && "".equals(tempDcRace.getSpBcsfp())) {
									flag = true;
								} else if (!"".equals(dcRace.getSpBcsfp()) && !"".equals(tempDcRace.getSpBcsfp())) {
									if(Double.compare(Double.parseDouble(dcRace.getSpBcsfp()), Double.parseDouble(tempDcRace.getSpBcsfp())) != 0) {
										flag = true;
										compareRaces.get(compareRaces.size() - 1).setSpBcsfp(tempDcRace.getSpBcsfp());
									}
								}
								if (flag) {
									errorList.add(dcRace.getMatchNum());
								}
							}
		...
		dcRaces = dcRaceService.fetchDcRaceListByPhase(phaseNo, FetcherType
				.getItem(fetcherType));
	...
	public String matchAlias() {
		...
		AliasMatchBean match = new AliasMatchBean();
		match.setLeagueLongName(dcRace.getMatchName());
		match.setMatchTime(CoreDateUtils.formatDate(dcRace.getMatchDate(), CoreDateUtils.DATETIME));
		match.setHomeTeamLongName(dcRace.getHomeTeam());
		match.setAwayTeamLongName(dcRace.getAwayTeam());
		match.setMatchId(dcRace.getFxId());
		matchs.add(match);
		...
		if (matchs != null && matchs.size() > 0) {
			...
			if (match.getMatchId() != null && match.getMatchId() != 0) {
				msg = "匹配成功";
				dcRace.setFxId(match.getMatchId());
			} else {
				msg = "匹配失败";
				rs.put("message", msg);
				writeRs(ServletActionContext.getResponse(), rs);
			...
			dcRace.setAwayTeam(match.getAwayTeamShortName());
			dcRace.setHomeTeam(match.getHomeTeamShortName());
			dcRace.setMatchName(match.getLeagueShortName());
			rs.put("message", msg);
			rs.put("dcRace", JSONObject.fromObject(dcRace));
			writeRs(ServletActionContext.getResponse(), rs);
			return Action.NONE;
		} else {
			msg = "匹配失败";
	...
	// 抓取结果SP值数据与对阵信息拼装
	private List<DcRace> packDcRaces(List<DcRace> dbDcRaces,
			List<DcRace> fetchDcRaces) {
		// 抓取数据遍历
		Map<Integer, DcRace> dcRacesMap = new HashMap<Integer, DcRace>();
		for (DcRace dcRace : fetchDcRaces) {
			dcRacesMap.put(dcRace.getMatchNum(), dcRace);
		}

		// 数据库数据遍历
		for (DcRace dcRace : dbDcRaces) {
			DcRace tempDcRace = dcRacesMap.get(dcRace.getMatchNum());
			if (tempDcRace == null) {
				continue;
			}
			if (dcRace.getStatus().getValue() != DcRaceStatus.RETURN_PRIZE
					.getValue()) {
				dcRace.setHalfScore(tempDcRace.getHalfScore());
				dcRace.setWholeScore(tempDcRace.getWholeScore());
				dcRace.setSpSfp(tempDcRace.getSpSfp());
				dcRace.setSpSxds(tempDcRace.getSpSxds());
				dcRace.setSpJqs(tempDcRace.getSpJqs());
				dcRace.setSpBf(tempDcRace.getSpBf());
				dcRace.setSpBcsfp(tempDcRace.getSpBcsfp());
			}
		}
		return dbDcRaces;
	}

	public String updateDcRaces() {
		logger.info("进入更新单场赛程");
		// 更新单场sp值数据
		if (pageResult != null && !pageResult.trim().isEmpty()) {
			ResultBean resultBean = dcRaceService.batchUpdateDcRaceSp(dcRaces);
			phaseNo = dcRaces.get(0).getPhase();
			try {
				searchPhaseList();
				this.phase = this.phaseService.getPhaseByPhaseTypeAndPhaseNo(
						phaseType, phaseNo);
		...
		initStatus(dcRaces);
		ResultBean resultBean = dcRaceService.batchCreatePhase(dcRaces);
		phaseNo = dcRaces.get(0).getPhase();
		try {
			searchPhaseList();
			this.phase = this.phaseService.getPhaseByPhaseTypeAndPhaseNo(
	...
	// 初始化对阵状态
	private void initStatus(List<DcRace> dcRaces) {
		for (DcRace dcRace : dcRaces) {
	...
	// AJAX修改单条单场信息
	public String updateDcRace() {
		logger.info("进入更新单场信息");
		boolean b = false;

		// 更新结果Sp数据
		try {
			if ("sp".equals(isSpOrRace) && dcRace != null) {
				b = dcRaceService.updateDcRaceSp(dcRace);
			} else if ("dcRace".equals(isSpOrRace) && dcRace != null) {
				b = dcRaceService.updateDcRace(dcRace);
			}
		} catch (Exception e) {
	...
	public String checkDcRace() {
		...
		try {
			...
			// 状态设为已开奖
			DcRaceStatus dcRaceStatus = DcRaceStatus.getItem(statusValue);
			if (dcRaceStatus != null && dcRaceStatus.getValue() == DcRaceStatus.STOP_BUY.getValue()) {//只有在比赛状态未已关闭时才更新为已开奖
				dcRaceService.updateStatus(dcRace.getId(), DcRaceStatus.OPEN_BUY);
			}
	...
	public String resetMatch() {
        ...
        // 所有方案列表
        List<Plan> allPlanList = new ArrayList<Plan>();
		...
		while (true) {
			...
			pageBean.setPage(i);
			try {
				tmpPlanList = lotteryPlanService.findByPhaseType(phaseType, phaseNo, planStatusList, pageBean);
			} catch (ApiRemoteCallFailedException e) {
				logger.error(e.getMessage(), e);

                super.setErrorMessage("[重算截止]获取方案列表时调用API异常");
                rc = 1;
                message = "[重算截止]获取方案列表时调用API异常";
                json.put("code", rc);
                json.put("message", message);
                writeRs(ServletActionContext.getResponse(), json);
                return Action.NONE;
            ...
            allPlanList.addAll(tmpPlanList);
	...
	public String updateStatus() {
		...
		if (tag.equals("1")) {// 停止销售
			dcRaceService.updateStatus(dcRace.getId(), DcRaceStatus.STOP_BUY);
		} else if (tag.equals("2")) { // 解除停止
			dcRaceService.updateStatus(dcRace.getId(), DcRaceStatus.CAN_BUY);
		} else if (tag.equals("3")) { // 恢复未销售
			dcRaceService.updateStatus(dcRace.getId(), DcRaceStatus.NO_BUY);
		}
		json.put("code", rc);
		json.put("message", message);
		writeRs(ServletActionContext.getResponse(), json);
	...
	public String getExcelDcRaces() {
		...
		List<Object> rtnList = dcRaceService.getDcRaceByExcel(excelFile, phase, phaseNo);
		...
		if (rtnList.get(0) instanceof List<?>) {
			dcRaces = (List<DcRace>) rtnList.get(0);
		}
	...
	public DcRace getDcRace() {
		return dcRace;
	}

	public void setDcRace(DcRace dcRace) {
		this.dcRace = dcRace;
	}
	...
	public List<DcRace> getCompareRaces() {
		return compareRaces;
	}

	public void setCompareRaces(List<DcRace> compareRaces) {
