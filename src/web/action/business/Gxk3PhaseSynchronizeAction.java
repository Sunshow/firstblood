 ...
 * 广西快3彩期同步
 */
public class Gxk3PhaseSynchronizeAction extends BaseAction {
	...
	//查询参数区
	private Integer lotteryTypeValue = LotteryType.GXK3.getValue();//广西快3
	...
	public String handle() {
		...
		try {
		    phases= phaseService.findVenderOnSalePhases(phaseType, BIService.SHJHT_Phase_Synchronous);
		} catch (ApiRemoteCallFailedException e) {
			logger.error("查询广西快3在售的彩期列表，api调用异常，{}",e.getMessage());
			super.setErrorMessage("api调用异常，请联系技术人员!原因：" + e.getMessage());
			return "failure";
		}
		if (phases == null || phases.size() == 0) {
			logger.info("广西快3在售的彩期列表为空");
		}
