...
public class BatchSendToInternalAction extends BaseAction {
	private static final long serialVersionUID = 1L;

	private static final Logger logger = LoggerFactory.getLogger(BatchSendToInternalAction.class);
	
	private MemberService memberService;
	private ManuallyRechargeService manuallyRechargeService;
	
	private static List<WalletType> walletTypeList;												//现金钱包类型列表
	...
	public String handle() {
		logger.info("进入批量派发内部员工账户上传excel文件页面");
		return "inputForm";
	}
	...
	public String confirm() {
		...
		for (int i = 1; i < sheet.getPhysicalNumberOfRows(); i++) {
			...
			Double amountD = null;
			try {
				amountD = sheet.getRow(i).getCell(1).getNumericCellValue();
			} catch (Exception e) {
	...
	public String manage() {
		...
		if(object  == null) {
			super.setErrorMessage("已发");
			return "failure";
		}
		getSession().remove("struts.token");
		logger.info("进入派送至内部账户");
		...
		ManuallyRechargeType manuallyRechargeType = ManuallyRechargeType.LEHECAI;					//手工充值类型
		WalletType walletType = WalletType.getItem(walletTypeValue);										//至内部账户钱包类型
		for (int i = 0; i < accounts.size(); i++) {
			...
			try {
				...
				logger.error("赠送至内部账户，api调用异常，{}", e.getMessage());
				resultMap.put(account, "赠送至内部账户时，api调用异常：" + e.getMessage());
				resultList.add(resultMap);
				continue;
			}
			
			logger.info("{}账户赠送至内部账户{}成功", account, CoreNumberUtil.formatNumBy2Digits(amount));
			resultMap.put(account, "赠送至内部账户" + CoreNumberUtil.formatNumBy2Digits(amount) + "成功");
			resultList.add(resultMap);
		}
		return "success";
	}
	...
	public List<WalletType> getWalletTypeList() {
		...
		walletTypeList.add(WalletType.LEHECAI);
