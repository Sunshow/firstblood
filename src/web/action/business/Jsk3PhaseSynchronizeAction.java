 ...
 * 江苏快3彩期同步
 */
public class Jsk3PhaseSynchronizeAction extends BaseAction {
	...
	//查询参数区
	private Integer lotteryTypeValue = LotteryType.JSK3.getValue();//江苏快3
	...
	public String findOnsalePhaseList() {
		...
		try {
			...
			logger.error("查询江苏快3在售的彩期列表，获取的彩期列表为null，{}",e.getMessage());
			super.setErrorMessage("获取彩期列表为失败，失败信息：" + e.getMessage());
			return "failure";
		}
		
		LotteryType lotteryType = LotteryType.JSK3;
		// 先查找当前彩期            
		...
		if (failedPhaseList != null && !failedPhaseList.isEmpty()) {
			...
			try {
				...
				if (updatePhaseList == null || updatePhaseList.isEmpty()) {
					throw new ApiRemoteCallFailedException("查找江苏快3彩期出错");
				}
			} catch (ApiRemoteCallFailedException e) {
				e.printStackTrace();
			}
