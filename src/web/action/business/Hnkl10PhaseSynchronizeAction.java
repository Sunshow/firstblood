...
public class Hnkl10PhaseSynchronizeAction extends BaseAction {
	...
	// 查询参数区
	private LotteryType lotteryType = LotteryType.HNKL10;// 湖南快乐十分

	/** 查询湖南快乐十分在售的彩期 */
	public String handle() {
		logger.info("进入查询湖南快乐十分在售的彩期列表");

		PhaseType phaseType = PhaseType.getItem(lotteryType);

		if (phaseType == null) {
			logger.error("未获取到湖南快乐十分对应彩期类型");
			super.setErrorMessage("未获取到湖南快乐十分对应彩期类型");
			return "failure";
		}

		// 开始查询
		try {
			phases = phaseService.findVenderOnSalePhases(phaseType, BIService.ZHX_Phase_Synchronous);
		} catch (ApiRemoteCallFailedException e) {
			logger.error("查询湖南快乐十分在售的彩期列表,api调用异常");
	...
	public String batchUpdate() {
		logger.info("进入批量修改湖南快乐十分彩期");
		if ((phaseStr == null || phaseStr.equals("")) || (startSaleTimeStr == null || startSaleTimeStr.equals("")) || (endSaleTimeStr == null || endSaleTimeStr.equals(""))
				|| (drawTimeStr == null || drawTimeStr.equals(""))) {
			logger.error("修改数据为空");
			super.setErrorMessage("修改数据不能为空");
			return "failure";
		}

		PhaseType phaseType = PhaseType.getItem(this.lotteryType);
