 ...
 * 
 */
public class NotifyCustomerServiceSupervisorDelegate extends AbstractDelegate {
	protected final static String PROCESS_ID = "addedRewardsProcess";
    protected final static String TASK_ID = "auditAddedRewardsTask";
	...
	*/
	...
	protected void doExecution(DelegateExecution execution) throws Exception {
		...
		for (WorkProcessUser workProcessUser : workProcessUsers) {
			...
			if (flag) {
				logger.info("需要通知客服主管"+workProcessUser.getName());
				logger.info("发送内容补充派奖工单创建通知:由[" + addedRewardsTaskForm.getAddedRewardsTask().getInitiator() + "]发起了一个补充派奖工单，方案编码为["
				...
				if(workProcessUser.isEmailNotify() && usersList.size() > 0){
					...
					String subject = "补充派奖工单创建通知";
					String text = "由[" + addedRewardsTaskForm.getAddedRewardsTask().getInitiator() + "]发起了一个补充派奖工单，方案编码为["
					+ addedRewardsTaskForm.getAddedRewardsTask()
							.getPlanId()
					+ "],彩种["
					+ addedRewardsTaskForm.getAddedRewardsTask()
				...
				if(workProcessUser.isSmsNotify() && usersList.size() > 0){
					...
					String text = "补充派奖工单创建通知:由[" + addedRewardsTaskForm.getAddedRewardsTask().getInitiator() + "]发起了一个补充派奖工单，方案编码为["
					+ addedRewardsTaskForm.getAddedRewardsTask()
							.getPlanId()
					+ "],彩种["
					+ addedRewardsTaskForm.getAddedRewardsTask()
