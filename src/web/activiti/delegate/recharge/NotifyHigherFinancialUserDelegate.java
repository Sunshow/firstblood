 ...
 *
 */
public class NotifyHigherFinancialUserDelegate extends AbstractDelegate {
	...
	protected void doExecution(DelegateExecution execution) throws Exception {
		...
		for (WorkProcessUser workProcessUser : workProcessUsers) {
			...
			if (flag) {
				...
				logger.info("发送内容汇款充值工单通知:财务人员额度不足需要上级财务人员处理：用户名[" + rechargeTaskForm.getRechargeTask().getUsername() + "]，真实姓名[" + rechargeTaskForm.getRechargeTask().getRealName() + "]，汇款金额为[" + rechargeTaskForm.getRechargeTask().getAmount() + "]元");
				...
				if(workProcessUser.isEmailNotify() && usersList.size() > 0){
					...
					String subject = "汇款充值工单通知";
					String text = "财务人员额度不足需要上级财务人员处理：用户名[" + rechargeTaskForm.getRechargeTask().getUsername() + "]，真实姓名[" + rechargeTaskForm.getRechargeTask().getRealName() + "]，汇款金额为[" + rechargeTaskForm.getRechargeTask().getAmount() + "]元";
				...
				if(workProcessUser.isSmsNotify() && usersList.size() > 0){
					...
					String text = "汇款充值工单通知:财务人员额度不足需要上级财务人员处理：用户名[" + rechargeTaskForm.getRechargeTask().getUsername() + "]，真实姓名[" + rechargeTaskForm.getRechargeTask().getRealName() + "]，汇款金额为[" + rechargeTaskForm.getRechargeTask().getAmount() + "]元";
					for(User user : usersList){
