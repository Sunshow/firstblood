 ...
 * 
 */
public class CeoHandleGiftRewardsTask extends CommonProcessTask {
	
	public static final String GIFT_REWARDS_PROCESS = "giftRewardsProcess";
	public static final String CEO_HANDLE_GIFT_REWARDS_TASK = "ceoHandleGiftRewardsTask";
	...
	public List<GiftRewardsTaskForm> listByRoleOrUser(String roleId, String userId) {
		...
		List<Task> allTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_REWARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_REWARDS_TASK).taskCandidateGroup(roleId).orderByTaskCreateTime().desc().list();
		if (allTaskList == null || allTaskList.size() == 0) {
			logger.info("处理彩金增送工单：查询userID={}用户的任务列表", userId);
			allTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_REWARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_REWARDS_TASK).taskCandidateUser(userId).orderByTaskCreateTime().desc().list();
		}
	...
	public void agreeTask(String taskId,Long opUserId){
		Map<String, Object> variables = new HashMap<String, Object>();
		variables.put("agree", true);
		variables.put("taskName", CEO_HANDLE_GIFT_REWARDS_TASK);
	...
	public List<GiftRewardsTaskForm> listByAssignee(String assignee) {
		...
		List<Task> approvedTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_REWARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_REWARDS_TASK).taskAssignee(assignee).list();
		List<GiftRewardsTaskForm> giftRewardsTaskFormList = new ArrayList<GiftRewardsTaskForm>();
