...
public class CeoHandleCommissionTask extends CommonProcessTask {
	
	public static final String COMMISSION_PROCESS = "commissionTaskProcess";
	public static final String CEO_HANDLE_COMMISSION_TASK = "ceoHandleCommissionTask";
	...
	public List<CommissionTaskForm> listByRoleOrUser(String roleId, String userId) {
		...
		List<Task> allTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(CEO_HANDLE_COMMISSION_TASK).taskCandidateGroup(roleId).orderByTaskCreateTime().desc().list();
		if (allTaskList == null || allTaskList.size() == 0) {
			logger.info("处理佣金派发工单：查询userID={}用户的任务列表", userId);
			allTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(CEO_HANDLE_COMMISSION_TASK).taskCandidateUser(userId).orderByTaskCreateTime().desc().list();
		}
	...
	public void agreeTask(String taskId){
		Map<String, Object> variables = new HashMap<String, Object>();
		variables.put("agree", true);
		variables.put("taskName", CEO_HANDLE_COMMISSION_TASK);
	...
	public List<CommissionTaskForm> listByAssignee(String assignee) {
		...
		List<Task> approvedTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(CEO_HANDLE_COMMISSION_TASK).taskAssignee(assignee).list();
