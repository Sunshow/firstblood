...
public class SupervisorHandleCommissionTask extends CommonProcessTask {
	
	public static final String COMMISSION_PROCESS = "commissionTaskProcess";
	public static final String SUPERVISOR_HANDLE_COMMISSION_TASK = "supervisorHandleCommissionTask";
	...
	public List<CommissionTaskForm> listByRoleOrUser(String roleId, String userId) {
		...
		List<Task> allTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(SUPERVISOR_HANDLE_COMMISSION_TASK).taskCandidateGroup(roleId).orderByTaskCreateTime().desc().list();
		if (allTaskList == null || allTaskList.size() == 0) {
			logger.info("处理佣金派发工单：查询userID={}用户的任务列表", userId);
			allTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(SUPERVISOR_HANDLE_COMMISSION_TASK).taskCandidateUser(userId).orderByTaskCreateTime().desc().list();
		}
	...
	public void agreeTask(boolean enough, String taskId,Long opUserId){
		...
		variables.put("taskName", SUPERVISOR_HANDLE_COMMISSION_TASK);
	...
	public List<CommissionTaskForm> listByAssignee(String assignee) {
		...
		List<Task> approvedTaskList = taskService.createTaskQuery().processDefinitionKey(COMMISSION_PROCESS).taskDefinitionKey(SUPERVISOR_HANDLE_COMMISSION_TASK).taskAssignee(assignee).list();
