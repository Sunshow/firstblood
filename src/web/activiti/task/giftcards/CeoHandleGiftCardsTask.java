 ...
 * 
 */
public class CeoHandleGiftCardsTask extends CommonProcessTask {
	
	public static final String GIFT_CARDS_PROCESS = "giftCardsProcess";
	public static final String CEO_HANDLE_GIFT_CARDS_TASK = "ceoHandleGiftCardsTask";
	...
	public List<GiftCardsTaskForm> listByRoleOrUser(String roleId, String userId) {
		...
		List<Task> allTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_CARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_CARDS_TASK).taskCandidateGroup(roleId).orderByTaskCreateTime().desc().list();
		if (allTaskList == null || allTaskList.size() == 0) {
			logger.info("处理彩金卡申请工单：查询userID={}用户的任务列表", userId);
			allTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_CARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_CARDS_TASK).taskCandidateUser(userId).orderByTaskCreateTime().desc().list();
		}
	...
	public List<GiftCardsTaskForm> listByAssignee(String assignee) {
		...
		List<Task> approvedTaskList = taskService.createTaskQuery().processDefinitionKey(GIFT_CARDS_PROCESS).taskDefinitionKey(CEO_HANDLE_GIFT_CARDS_TASK).taskAssignee(assignee).list();
		List<GiftCardsTaskForm> giftCardsTaskFormList = new ArrayList<GiftCardsTaskForm>();
