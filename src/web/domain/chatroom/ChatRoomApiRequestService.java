...
import com.lehecai.core.api.IApiRequest;
 ...
 *
 */
public class ChatRoomApiRequestService {
	...
	public ChatRoomApiResponse request(ApiConfig apiConfig, IApiRequest apiRequest, int timeout_msec) throws ApiRemoteCallFailedException {
		if (apiRequest == null) {
			return null;
		}
		...
		try {
			...
			*/
			...
			response.setMessage(json.getString(ChatRoomApiConstant.API_RESPONSE_MESSAGE_NAME));
			if (!StringUtils.isEmpty(response.getMessage())) {
				logger.info("返回消息，message={}", response.getMessage());
			}else{
				if (json.containsKey(ChatRoomApiConstant.API_RESPONSE_DATA_NAME)) {
					try {
						JSONObject data = json.getJSONObject(ChatRoomApiConstant.API_RESPONSE_DATA_NAME);
						response.setData(data);
						if(data.containsKey(ChatRoomApiConstant.API_RESPONSE_COUNT)){
							response.setCount(data.getInt(ChatRoomApiConstant.API_RESPONSE_COUNT));
							response.setArrayData(data.getJSONArray(ChatRoomApiConstant.API_RESPONSE_ARRAY_DATA_NAME));
						}
					} catch (Exception e) {
						this.increaseFailedCount();
...
/*			if (response.getCode() != ApiConstant.RC_SUCCESS) {
	...
	public ChatRoomApiResponse request(IApiRequest apiRequest, int timeout_msec) throws ApiRemoteCallFailedException {
		return this.request(this.apiConfig, apiRequest, timeout_msec);
	}
	
	public ChatRoomApiResponse request(IApiRequest apiRequest) throws ApiRemoteCallFailedException {
