...
public class RewardServiceImpl implements RewardService {
	...
	private final static String PROCESS_CODE = "60001";
	...
	public JSONObject getRewardStatus(LotteryType lotteryType, String taskId) {
		if (lotteryType == null) {
			logger.error("获取派奖状态获取lotteryType参数不能为空");
			return null;
		}
		...
		try {
			...
			logger.error("获取派奖状态获取engine调用地址失败");
		...
		try {
			list = CoreHttpUtils.getUrl(engineUrl + SERVLET_URL,
					"ProcessCode=" + PROCESS_CODE + "&lotteryTypeId="
							+ lotteryType.getValue() + "&taskId=" + taskId
							, CharsetConstant.CHARSET_UTF8, 5000);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}

		if (list == null || list.size() == 0) {
			logger.error("获取派奖状态lotteryType={}的方案时，返回值为空", lotteryType
					.getName());
			return null;
		}
		Map<String, String> resultMap = CoreHttpUtils.parseQueryString(list
				.get(0), CharsetConstant.CHARSET_UTF8);
		if (resultMap == null || !resultMap.containsKey(KEY_CODE)
				|| !resultMap.containsKey(KEY_MESSAGE)) {
			logger.error("获取派奖状态lotteryType={}的方案时，返回值数据结构错误", lotteryType
					.getName(), list.get(0));
			return null;
		}
		if (Integer.parseInt(resultMap.get(KEY_CODE)) != 0) {
			logger.error("获取派奖状态lotteryType={}的方案失败，原因：{}", lotteryType
					.getName(), resultMap.get(KEY_MESSAGE));
			return null;
		}
